...
from execute import PyExecutor
#from generators import , model_factory
from generators.py_generator import PyGenerator
...
def run_mcts(
    ...
    for idx, data in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        while cur_func_impl is None:
            cur_func_impl = gen.func_impl(data['description'], model, "simple")
        ...
        is_passing, feedback, _ = exe.execute(
                                                ...
                                                program_id = data["problem_id"],
                                                num_tests = int(data["num_tests"]*test_percent),
                                                timeout=5
                                             )
        ...
        if is_passing:
            ...
            num_success += 1
            data["acc"] = round(num_success/(idx+1), 2)
            data["prediction"] = cur_func_impl
            write_jsonl(log_path, [data], append=True)
            print(num_success)
            print(f'completed {idx+1}/{num_items}: acc = {round(num_success/(idx+1), 2)}')
            continue
        ...
        #break
        for cur_iter in trange(max_iters, desc="running max_iters"):
            # Selection
            #tests_i = gen.internal_tests(item["prompt"], test_model, number_of_tests)
            ...
            for _ in range(n):
                ...
                for child in node.children:
                    ...
                    if "Tests passed:" in feedback_internal:
                        ...
                        reward_internal = len([line for line in passed_section.split("Tests passed:")[1].splitlines() if line.strip() != ''])
                        reward_internal = reward_internal / int(data["num_tests"]*test_percent)
                    else:
                        reward_internal = 0
                    if is_passing_internal or cur_iter == max_iters - 1:
                        #is_passing = exe.evaluate(data["entry_point"], child.solution, item["test"], timeout=10)
                        is_passing, feedback, _ = exe.execute(
                                                    program = child.solution,
                                                    program_id = data["problem_id"],
                                                    num_tests = data["num_tests"],
                        ...
                        if is_passing:
                            data["prediction"] = child.solution
                            is_solved = True
                            reward_real = 1
                        break
        ...
        data["is_solved"] = is_passing
        data["reflections"] = reflections
        data["implementations"] = implementations
        data["test_feedback"] = test_feedback
        data["acc"] = round(num_success/(idx+1), 2)
        write_jsonl(log_path, dataset, append=False)
